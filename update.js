...
function updateToday() {
    ...
    var url = 'data/' + dateFormat + "/" + currentStationID + ".txt";
    let lineLength = 34;
...
function updateTopTens() {
    ...
    let lineLength = 34;
    let varLengths = {temp: [0, 3], prcp: [20, 23], cldc: [28, 31]};
    let calls = [];
    $.each(code_stations, function (i, v) {
        let code = v.id;
        var url = 'data/' + dateFormat + "/" + code + ".txt";
        let call = $.ajax({
            type: 'GET',
            url: url,
            dataType: 'text',
    ...
    $.when.apply(null, calls).then(function () {
        let topTenTemp = getTopTen(topTemps);
        // let topTenRain = getTopTen(topRains);
        // let topTenCloud = getTopTen(topClouds);
    ...
    function getTopTen(topValues){
        var items = Object.keys(topValues).map(function (key) {
    ...
    function showTopTen(topTen, type) {
        ...
        $.each(topTenArrayNames, function () {
            ...
            $(value_li).html(value.toFixed(1) + extension);
            index++;
        });
    }
}
